 ...
 */
const getModuleInfo = async (filePath: string, apiSourcePath: string): Promise<ModuleInfo | null> => {
  ...
  const usedTypes = mainMethodParamNodes.reduce<string[]>(
    (t, n) => [...t, ...getParamsTypes(n.getChildren(sourceFile), sourceFile)],
    [],
  );
  usedTypes.push(...getParamsTypes([mainMethodReturnTypeInfo.typeNode!], sourceFile));
